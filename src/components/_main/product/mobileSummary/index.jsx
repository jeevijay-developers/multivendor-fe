...
Incrementer.propTypes = { available: PropTypes.number.isRequired };
export default function ProductDetailsSumaryMobile({ ...props }) {
  ...
  const isMaxQuantity =
    !isLoading &&
    checkout.cart.filter((item) => item._id === product?._id).map((item) => item.quantity)[0] >= product?.stockQuantity;
      ...
      try {
        ...
        if (!Boolean(alreadyProduct.length)) {
          ...
          onAddCart({
            ...
            subtotal: (product.salePrice || product?.price) * values.quantity
          });
        ...
        <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
          ...
          <Stack direction="row" alignItems="center" className="rating-wrapper" spacing={1}>
            ...
            <Typography variant="h4" className="text-price">
              ...
              <Box component="span">
                &nbsp;
                {!isLoading && isLoaded && fCurrency(product?.salePrice)}
              </Box>
            </Typography>
          </Stack>
            ...
            {product?.price > product?.salePrice && (
              <Stack direction="row" alignItems="center" spacing={1}>
                ...
                <Typography variant="subtitle1" color="text.secondary" fontWeight={400} className="text-discount">
                  {!isLoading && isLoaded && fCurrency(product?.price - product?.salePrice)}
                  {<span>({(100 - (product?.salePrice / product?.price) * 100).toFixed(0)}% Discount)</span>}
                </Typography>
              </Stack>
            )}
            <Stack direction="row" alignItems="center" spacing={1}>
              ...
              >
                {product?.stockQuantity > 0 ? `${product?.stockQuantity} Items` : <span>Out of stock</span>}
              </Typography>
            </Stack>
          </Stack>
          <Stack direction="row" alignItems="center" spacing={2} className="incrementer-wrapper">
            ...
            {isLoading ? (
              ...
              <div>
                <Incrementer name="quantity" available={product?.stockQuantity} />
          ...
          <Stack spacing={2} className="detail-actions-wrapper">
            <Stack spacing={2} direction={{ xs: 'row', sm: 'row' }} className="contained-buttons">
              <Button
                fullWidth
                disabled={isMaxQuantity || isLoading || product?.stockQuantity < 1}

...
const getSearchParams = (searchParams, category, subCategory, childCategory, brand, shop, campaign, rate) => {
  ...
  if (campaign?._id) params.set('campaign', campaign.slug);
  if (campaign?._id) params.set('campaign', campaign.slug);
...
export default function ProductListing({ category, subCategory, childCategory, shop, campaign, brand, filters }) {
  const searchParams = useSearchParams();
  const { rate } = useSelector(({ settings }) => settings);
  const searchQuery = getSearchParams(searchParams, category, subCategory, childCategory, brand, shop, campaign, rate);
  const { data, isPending: isLoading } = useQuery({
    queryKey: [searchQuery, rate],

...
export default function AccountBillingInvoiceHistory() {
  ...
  const { data, isPending: isLoading } = useQuery({ queryKey: ['invoices'], queryFn: api.getInvoices });
  return (
    <Card sx={{ p: 3, width: 1, position: 'sticky', top: '80px', display: { md: 'block', xs: 'none' } }}>

...
export default function VariableProduct({
  ...
  const handleDrop = (acceptedFiles, index) => {
    ...
    const uploaders = acceptedFiles.map((file) => {
      ...
      setFieldValue(`variants[${index}].blob`, values.blob.concat(acceptedFiles));
  ...
  const handleRemove = (file, index) => {
    const removeImage = values.variants[index].images.filter((_file) => {
      if (_file._id === file._id) {
        deleteMutate(file._id);
      }
      return _file !== file;
    });
    setFieldValue(`variants[${index}].images`, removeImage);
  };
